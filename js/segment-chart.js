function SegmentChart(id, context, center_x, center_y, inner_radius, thickness, initial_angle = -90, angle = 360) {    
    ...
    this.type = 'pie'; // pie, radial, bar
    this.source = []; // array of arrays [ 'key', 'value', 'color' ]
    this.data = [];
    
    this.min_value = 0; // 0%
    this.max_value = 100; // 100%
    this.total = 0;
    ...
    this.segment_angle = 0;
    this.segment_position = '';
    this.segment_thickness = 0.8 * this.thickness;
    ...
    this.spaces_count = 0;
    this.space_angle = 0;    
    this.space_thickness = 3;
    
    this.signs = [];
    this.sign_r_in = this.r_in + this.thickness + 10;
    this.sign_r_offset = 10;
    ...
    this.text_direction = 'vertical'; // vertical, clockwise, anticlockwise, from-center, to-center
...
SegmentChart.prototype.build = function() {
    this.segments = [];
...
SegmentChart.prototype.build_pie = function() {
...
SegmentChart.prototype.build_radial = function() {
...
SegmentChart.prototype.build_bar = function() {
    let r_in = this.r_in;
...
SegmentChart.prototype.strToValue = function(str) {
    let value;
...
SegmentChart.prototype.valueToAngle = function(val) {
    let angle;
...
SegmentChart.prototype.valueToThickness = function(val) {
    let thickness;
...
SegmentChart.prototype.calc = function() {
    if(this.in_progress) {
...
SegmentChart.prototype.draw = function() {
...
SegmentChart.prototype.appear = function(order, lag, direction, duration, delay) {
...
SegmentChart.prototype.checkAppearedSegments = function(segment) {
...
SegmentChart.prototype.disappear = function(order, lag, direction, duration, delay) {
    this.in_progress = true;
...
SegmentChart.prototype.checkDisappearedSegments = function(segment) {  
    if(this.segments.indexOf(segment) < 0) {
        return;
    }
    else {  
...
SegmentChart.prototype.fadeIn = function(order, lag, duration, delay) {
    this.in_progress = true;
...
SegmentChart.prototype.checkFadedInSegments = function(segment) {
    if(this.segments.indexOf(segment) < 0) {
        return;
    }
    else {    
...
SegmentChart.prototype.fadeOut = function(order, lag, duration, delay) {
    this.in_progress = true;
...
SegmentChart.prototype.checkFadedOutSegments = function(segment) {
    if(this.segments.indexOf(segment) < 0) {
        return;
    }
    else {    
